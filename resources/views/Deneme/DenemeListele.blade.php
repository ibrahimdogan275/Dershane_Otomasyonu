...
<div class="container">
    <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
        ...
        @foreach($denemeListele as $deneme )
            <tr>
                <td><a href="denemegir/{{$deneme->id}}/edit"><button type="submit" value="Güncelle" class="yon btn btn-primary" ><i class="fa fa-pencil-square-o" aria-hidden="true"></i>

                        </button></a>
                    <form method="post" action="/denemegir/{{$deneme->id}}">
                        {{csrf_field()}}
                        <input type="hidden"name="_method" value="DELETE">
                        <button type="submit" value="SİL" class="sil btn btn-danger" onclick="return confirmDel('denemegir/{{$deneme->id}};')"><i class="fa fa-trash" aria-hidden="true"></i></button>
                        </a>
                    </form>
